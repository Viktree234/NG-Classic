  ...
  return (
    <>
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <h1 className="font-playfair text-3xl text-heading mb-8">Checkout</h1>

        {/* Order summary */}
        <div className="bg-surface-soft rounded-2xl p-6 mb-8">
          <h2 className="font-medium text-heading mb-4">Order Summary</h2>
          {items.map(item => (
            <div key={item.id} className="flex justify-between text-sm text-secondary py-1">
              <span>{item.name} × {item.qty}</span>
              <span>₦{(item.price * item.qty).toLocaleString()}</span>
            </div>
          ))}
          <div className="border-t border-border-default mt-4 pt-4 flex justify-between font-semibold text-heading">
            <span>Total</span>
        ...
        <div className="space-y-4 mb-8">
          <h2 className="font-medium text-heading">Shipping Details</h2>
        ...
        {scriptError && (
          <p className="mt-4 text-sm text-red-600 dark:text-red-400">{scriptError}</p>
        )}

        {!user && (
          <p className="text-center text-sm text-secondary mt-6">
            <Link href="/login" className="text-heading-strong hover:underline">Log in</Link> to track your orders.
          </p>
        )}
      </div>
    </>
